...
import { GameEngineFP } from '@/gamefp/engine';
import type { GameModeFP, FPRaceResult, FPGUdateState } from '@/gamefp/types';

interface GameFPViewProps {
  mode: GameModeFP;
  onExit: () => void;
  onFinish: (results: FPRaceResult[]) => void;
}

export function GameFPView({ mode, onExit, onFinish }: GameFPViewProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<GameEngineFP | null>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [updateState, setUpdateState] = useState<FPGUdateState | null>(null);
  const [showExitConfirm, setShowExitConfirm] = useState(false);

  const handleUpdate = useCallback((state: FPGUdateState) => {
    setUpdateState(state);
  }, []);

  const handleResults = useCallback(
    (results: FPRaceResult[]) => {
      onFinish(results);
    },
    [onFinish]
  );
  ...
  useEffect(() => {
    ...
    const engine = new GameEngineFP(canvas, mode, handleResults, handleUpdate);
...
interface ResultsFPScreenProps {
  results: FPRaceResult[];
  onPlayAgain: () => void;
  onMenu: () => void;
}

export function ResultsFPScreen({ results, onPlayAgain, onMenu }: ResultsFPScreenProps) {
  return (
    <div className="min-h-screen w-full relative overflow-hidden bg-gradient-to-b from-slate-900 via-teal-950 to-emerald-950 flex items-center justify-center px-4">
      {/* Confetti */}
        ...
        {Array.from({ length: 50 }).map((_, i) => (
          <div
            ...
            style={{
              ...
              top: `${Math.random() * 100}%`,
              animation: `confettiFP ${2 + Math.random() * 3}s linear infinite`,
      ...
      {/* Glow */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-96 h-96 rounded-full bg-teal-500/15 blur-3xl pointer-events-none" />

      <div className="relative z-10 max-w-lg w-full">
        {/* Header */}
        <div className="text-center mb-8 animate-[bounceFP_0.6s_ease-out]">
          <div className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-b from-teal-300 to-emerald-500 drop-shadow-lg">
            FINISH
          </div>
        ...
        <div className="space-y-3 mb-8">
          {results.map((result, i) => (
            <div
              key={result.name}
              className={`flex items-center gap-4 p-4 rounded-2xl border transition-all animate-[slideFP_0.5s_ease-out_${i * 0.15}s_both] ${
                i === 0
                  ? 'bg-gradient-to-r from-teal-500/25 to-emerald-600/15 border-teal-500/40'
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: i === 0 ? '#2dd4bf' : i === 1 ? '#94a3b8' : i === 2 ? '#b45309' : '#374151',
                  color: i === 0 ? '#000' : i < 3 ? '#000' : '#fff',
                }}
              >
                {i + 1}
              </div>
              ...
              <div className="flex-1 text-right">
                ...
                <div className="text-teal-400 text-xs">Top Speed: {result.topSpeed} km/h</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onPlayAgain}
            className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-gradient-to-r from-teal-500 to-emerald-500 text-white font-bold hover:from-teal-400 hover:to-emerald-400 transition-all active:scale-95"
          >
            Race Again
          </button>
          <button
