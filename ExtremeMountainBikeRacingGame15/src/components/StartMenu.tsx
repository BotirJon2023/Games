...
import { Mountain, Trophy, Cpu, Users, Gauge, Zap, Bike, Sunset, Eye, Flower2 } from 'lucide-react';

export type GameStyle = '2d' | '3d' | 'fp' | 'jp';
        ...
        {step === 'style' ? (
          ...
          <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-8 animate-[fadeInUp_0.8s_ease-out_0.2s_both]">
            ...
            <StyleCard
              ...
              title="JAPAN COUNTRYSIDE"
              subtitle="Side-view race past Mt. Fuji, sakura trees & shrines, with tricks and boost"
              accent="from-rose-500 to-pink-800"
              highlighted={hoveredStyle === 'jp'}
              onHover={() => setHoveredStyle('jp')}
              onLeave={() => setHoveredStyle(null)}
              onClick={() => handleStyleSelect('jp')}
              badge="Side View"
            />
          </div>
        ) : (
        ...
        {step === 'style' ? (
          <div className="bg-black/30 backdrop-blur-sm rounded-2xl px-6 py-5 max-w-2xl w-full border border-white/10 animate-[fadeInUp_0.8s_ease-out_0.4s_both]">
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-amber-200 text-sm font-semibold">
                Four completely different racing experiences! Try each one for a unique gameplay style.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs text-sky-100">
              ...
              <div className="flex items-start gap-2">
                <Flower2 className="w-4 h-4 text-rose-400 flex-shrink-0 mt-0.5" />
                <p><span className="font-bold text-rose-300">Japan Countryside:</span> Sakura, Mt. Fuji & shrines with trick scoring and boost</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-black/30 backdrop-blur-sm rounded-2xl px-6 py-5 max-w-2xl w-full border border-white/10 animate-[fadeInUp_0.8s_ease-out_0.2s_both]">
            <h3 className="text-white font-bold text-sm mb-3 flex items-center gap-2">
              ...
              CONTROLS {selectedStyle === '3d' ? '(3D Arcade)' : selectedStyle === 'fp' ? '(Driver View)' : selectedStyle === 'jp' ? '(Japan Countryside)' : '(2D Side-Scroller)'}
            </h3>
            ...
            <div className="mt-3 pt-3 border-t border-white/10 flex items-center gap-2 text-xs text-amber-300">
              ...
              <span>
                {selectedStyle === '3d'
                  ...
                  : selectedStyle === 'jp'
                  ? 'Race past Mt. Fuji and sakura groves — land flips for trick points and time your boost!'
