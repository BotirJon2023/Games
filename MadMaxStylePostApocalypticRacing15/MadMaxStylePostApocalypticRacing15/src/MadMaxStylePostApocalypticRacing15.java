...
public class MadMaxStylePostApocalypticRacing15 {
    public static void main(String[] args) {
        JFrame frame = new JFrame("Apocalypse Arena — Ruined Coliseum Desert Racing");
...
class Particle {
    ...
    boolean update(double dt) {
        ...
        if (kind == ParticleKind.FIREWORK) {
            vy += 40 * dt; // slight gravity for celebratory sparks
            vx *= 0.99;
            vy *= 0.99;
        } else {
            vx *= 0.98;
...
/**
 * Rounded-square "coliseum floor" loop track generated from a superellipse:
 * |x/a|^n + |y/b|^n = 1. Higher n (~6) gives sharper near-corners and a
 * bigger flat/open infield than a typical rounded oval.
 *
 * Lap counting and edge rendering/collision both reuse the same proven
 * technique: a centerline point list with a parallel cumulative arc-length
 * array (robust lap progress even if the car strays off-centerline) and
 * per-point normals for offset-based inner/outer edges.
 */
class Track {
    ...
    /** A point `depth` units further inward (toward the infield) than the inner track edge, at index. */
    ...
    void draw(Graphics2D g) {
        ...
        g.setColor(new Color(150, 138, 116));
        g.fill(road);

        // worn stone flagstone seams across the road
        g.setColor(new Color(126, 114, 96, 130));
...
class Obstacle {
    ...
    void draw(Graphics2D g) {
        ...
        switch (kind) {
            case RUBBLE:
                g.setColor(new Color(110, 104, 96));
                g.fillOval((int) -radius, (int) -radius, (int) (radius * 2), (int) (radius * 1.6));
                g.setColor(new Color(80, 74, 66));
...
/** Purely decorative ruined coliseum dressing around the outer rim: no collision. */
class Ruin {
    enum Kind { PILLAR, BROKEN_PILLAR, STATUE }
    final Kind kind;
    final double x, y, lean, scale;

    Ruin(Kind kind, double x, double y, double lean, double scale) {
    ...
    void draw(Graphics2D g) {
        ...
        switch (kind) {
            case PILLAR:
                g.setColor(new Color(70, 66, 60, 190));
                g.fillRect(-10, -70, 20, 70);
                g.fillRect(-14, -78, 28, 10);
                g.fillRect(-13, 0, 26, 8);
                g.setColor(new Color(40, 38, 34, 190));
                for (int i = -8; i <= 8; i += 8) g.drawLine(i, -68, i, 0);
                break;
            case BROKEN_PILLAR:
                g.setColor(new Color(66, 62, 56, 190));
                Path2D.Double jag = new Path2D.Double();
                jag.moveTo(-9, 0);
                jag.lineTo(-9, -30);
                jag.lineTo(-2, -40);
                jag.lineTo(6, -32);
                ...
                jag.closePath();
                g.fill(jag);
                g.setColor(new Color(38, 36, 32, 190));
                g.drawLine(-9, -10, 9, -10);
                break;
            case STATUE:
                g.setColor(new Color(60, 58, 54, 190));
                g.fillRoundRect(-14, -6, 28, 10, 4, 4); // base
                g.fillRoundRect(-6, -50, 12, 46, 4, 4); // body
                g.fillOval(-7, -62, 14, 14); // head
                g.fillRect(-16, -44, 8, 6); // arm stub
                g.fillRect(8, -44, 8, 6);
                break;
        }
        g.setTransform(old);
...
/** Two torn banners hung around the rim with a slow, simple waving animation. */
class Banner {
    final double x, y, w, h, phase;
    final Color cloth;

    Banner(double x, double y, double w, double h, double phase, Color cloth) {
...
/**
 * Signature hazard: a wrecking ball on a rigid arm that swings in a slow full
 * circle around a fixed pivot in the infield. The pivot sits just inside the
 * inner track edge so that once per revolution the ball's swept circle clips
 * across into the racing lane on that side of the track.
 */
class WreckingBall {
    final double pivotX, pivotY, armLength, ballRadius;
    double angle;
    final double angularSpeed;

    WreckingBall(double pivotX, double pivotY, double armLength, double ballRadius, double angularSpeed) {
        this.pivotX = pivotX; this.pivotY = pivotY;
        this.armLength = armLength; this.ballRadius = ballRadius;
    ...
    double ballX() { return pivotX + Math.cos(angle) * armLength; }
    double ballY() { return pivotY + Math.sin(angle) * armLength; }

    void draw(Graphics2D g) {
        double bx = ballX(), by = ballY();

        g.setColor(new Color(90, 84, 74));
        g.fillOval((int) pivotX - 12, (int) pivotY - 12, 24, 24);
        g.setColor(new Color(50, 46, 40));
        g.drawOval((int) pivotX - 12, (int) pivotY - 12, 24, 24);

        g.setStroke(new BasicStroke(3));
        g.setColor(new Color(70, 68, 64));
        int links = 8;
        for (int i = 0; i <= links; i++) {
            double t = i / (double) links;
            double lx = pivotX + (bx - pivotX) * t;
            double ly = pivotY + (by - pivotY) * t;
            g.fillOval((int) lx - 3, (int) ly - 3, 6, 6);
        }
        g.setStroke(new BasicStroke(1));

        g.setColor(new Color(35, 33, 30));
        g.fillOval((int) (bx - ballRadius), (int) (by - ballRadius), (int) (ballRadius * 2), (int) (ballRadius * 2));
        g.setColor(new Color(90, 86, 80));
        g.fillOval((int) (bx - ballRadius * 0.4), (int) (by - ballRadius * 0.6), (int) (ballRadius * 0.7), (int) (ballRadius * 0.6));
        g.setColor(new Color(15, 14, 12));
        g.drawOval((int) (bx - ballRadius), (int) (by - ballRadius), (int) (ballRadius * 2), (int) (ballRadius * 2));
        for (int i = 0; i < 6; i++) {
            double sa = i * Math.PI / 3 + angle;
            int sx = (int) (bx + Math.cos(sa) * ballRadius * 0.8);
            int sy = (int) (by + Math.sin(sa) * ballRadius * 0.8);
...
class Car {
    ...
    private void emitDust(double dt, boolean onTrack) {
        ...
        Color c = onTrack ? new Color(200, 190, 160, 120) : new Color(170, 150, 110, 160);
...
class AIDriver {
    ...
    InputState think(double dt, List<Obstacle> obstacles, WreckingBall ball) {
        ...
        if (ball != null) {
            double dx = ball.ballX() - car.x, dy = ball.ballY() - car.y;
            double dist = Math.hypot(dx, dy);
            if (dist < 130) {
                double angToObs = Util.normalizeAngle(Math.atan2(dy, dx) - car.angle);
...
class GamePanel extends JPanel implements Runnable, KeyListener {
    ...
    private void setupWorld() {
        track = new Track(W / 2.0, H / 2.0 + 10, 430, 220, 6.0, 90);
        ...
        // Broken pillars & statues ringing the outer rim.
        ruins.clear();
        Random rr = new Random(99);
        for (int i = 0; i < n; i += 20) {
            Point2D.Double p = track.pointAtOffset(i, -track.innerSign * (track.halfWidth + 55 + rr.nextInt(25)));
            double lean = (rr.nextDouble() - 0.5) * 0.3;
            double scale = 0.8 + rr.nextDouble() * 0.5;
            double roll = rr.nextDouble();
            Ruin.Kind kind = roll < 0.45 ? Ruin.Kind.PILLAR : roll < 0.8 ? Ruin.Kind.BROKEN_PILLAR : Ruin.Kind.STATUE;
            ruins.add(new Ruin(kind, p.x, p.y, lean, scale));
        }

        // Two torn banners flapping among the ruins.
        banners.clear();
        Point2D.Double b1 = track.pointAtOffset(n / 8, -track.innerSign * (track.halfWidth + 70));
        Point2D.Double b2 = track.pointAtOffset((5 * n) / 8, -track.innerSign * (track.halfWidth + 70));
        banners.add(new Banner(b1.x - 20, b1.y - 60, 46, 70, 0.0, new Color(140, 40, 34)));
        banners.add(new Banner(b2.x - 20, b2.y - 60, 46, 70, 2.4, new Color(70, 60, 110)));

        // Wrecking ball: pivot placed just inside the west inner edge so the
        // swing occasionally clips across into the racing lane on that side.
        int westIdx = n / 2;
        Point2D.Double pivot = track.inwardPoint(westIdx, 80);
        wreckingBall = new WreckingBall(pivot.x, pivot.y, 105, 20, 0.55);
    }
    ...
    private void updateRace(double dt) {
        ...
        InputState in2;
        if (vsAI) {
            in2 = aiDriver.think(dt, obstacles, wreckingBall);
        } else {
            in2 = new InputState();
        ...
        resolveWreckingBallCollision(p1);
        resolveWreckingBallCollision(p2);

        // Purely celebratory: fireworks whenever a car crosses the start/finish line.
    ...
    /** Signature hazard collision: the wrecking ball's current swung position acts as a moving circular obstacle. */
    private void resolveWreckingBallCollision(Car c) {
        if (!c.alive) return;
        double bx = wreckingBall.ballX(), by = wreckingBall.ballY();
        double dx = c.x - bx, dy = c.y - by;
        double dist = Math.hypot(dx, dy);
        double minDist = c.radius() + wreckingBall.ballRadius;
        if (dist < minDist && dist > 0.0001) {
            double nx = dx / dist, ny = dy / dist;
            double overlap = minDist - dist;
            c.x += nx * overlap;
            c.y += ny * overlap;
            double impact = Math.max(80, Math.abs(c.speed));
            c.damage(impact * 0.22 + 15);
            c.speed *= -0.4;
            c.spinOut(1.1);
            for (int i = 0; i < 16; i++) {
                double ang = Math.random() * Math.PI * 2;
                ...
                particles.add(new Particle(c.x, c.y, Math.cos(ang) * sp, Math.sin(ang) * sp,
                        0.4 + Math.random() * 0.3, 4, 4, new Color(200, 190, 170), Particle.ParticleKind.DEBRIS));
            }
        }
    }
    ...
    protected void paintComponent(Graphics g0) {
        ...
        drawBackground(sg);
        for (Ruin ru : ruins) ru.draw(sg);
        track.draw(sg);
        for (Obstacle o : obstacles) o.draw(sg);
        wreckingBall.draw(sg);
        for (Banner ban : banners) ban.draw(sg, timeAccum);
        particles.draw(sg);
    ...
    private void drawBackground(Graphics2D g) {
        g.setPaint(new GradientPaint(0, 0, new Color(210, 150, 100), 0, (float) (H * 0.42), new Color(224, 196, 160)));
        g.fillRect(0, 0, W, (int) (H * 0.42));

        double pulse = 1 + 0.03 * Math.sin(timeAccum * 1.5);
        int sunR = (int) (55 * pulse);
        g.setPaint(new RadialGradientPaint(new Point(W - 200, 110), sunR * 3,
                new float[]{0f, 0.4f, 1f},
                new Color[]{new Color(255, 235, 200, 210), new Color(230, 170, 110, 90), new Color(230, 170, 110, 0)}));
        g.fillOval(W - 200 - sunR * 3, 110 - sunR * 3, sunR * 6, sunR * 6);
        g.setColor(new Color(255, 240, 215));
        g.fillOval(W - 200 - sunR / 2, 110 - sunR / 2, sunR, sunR);

        // Distant ruined coliseum wall silhouette (jagged crenellations instead of mountains).
        ...
        g.setPaint(new GradientPaint(0, (float) (H * 0.4), new Color(196, 176, 140), 0, H, new Color(150, 132, 104)));
    ...
    private void drawMenu(Graphics2D g) {
        ...
        centerText(g, "APOCALYPSE ARENA", H / 2 - 140);
        g.setFont(new Font("SansSerif", Font.BOLD, 20));
        g.setColor(Color.WHITE);
        centerText(g, "Ruined coliseum desert racing — mind the wrecking ball", H / 2 - 90);
