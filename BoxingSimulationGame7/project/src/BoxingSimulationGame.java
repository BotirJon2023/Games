...
public class BoxingSimulationGame extends Canvas implements Runnable {
    ...
    private Font uiFont = new Font("Arial", Font.PLAIN, 18);
    private Font bigFont = new Font("Arial", Font.BOLD, 64);

    public static void main(String[] args) {
        EventQueue.invokeLater(() -> {
            JFrame frame = new JFrame("Boxing Simulation Game - Mexican Pyramid Edition (AWT/Swing)");
    ...
    private void drawRing(Graphics2D g) {
        ...
        Color[] cols = new Color[]{new Color(60,70,90), new Color(35,40,55)};
    ...
    private void spawnHitSpark(double x, double y, boolean right) {
        for (int i=0;i<8;i++) {
            double a = (right ? 0 : Math.PI) + (rng.nextDouble()-0.5)*0.8;
    ...
    class Boxer {
        ...
        void update(double dt) {
            ...
            switch (state) {
                ...
                default -> { /* IDLE/MOVE managed via velocity */ }
            }
    ...
    static class AIController {
        ...
        void holdGuard(boolean on) {
            if (on) self.startGuard(); else self.stopGuard();
        }
    }
    ...
    class Background {
        ...
        private double sunT = 0;
        private double camBobT = 0;
        private double shimmerT = 0;

        Background() {
            for (int i=0;i<8;i++) clouds.add(new Cloud(rand(0, WIDTH), rand(60, 220), rand(90, 200), rand(0.08, 0.16)));
            for (int i=0;i<6;i++) birds.add(new Bird(rand(0, WIDTH), rand(140, 300), rand(80, 140), rng.nextBoolean()));
        }
        ...
        void update(double dt) {
            ...
            for (Cloud c : clouds) if (c.x > WIDTH + 200) { c.x = -200; c.y = rand(60, 240); }
            for (Bird b : birds) {
        ...
        void draw(Graphics2D g, double time, double shakeX, double shakeY) {
            ...
            double sunY = HEIGHT * 0.18;
            float[] rf = new float[]{0f, 1f};
            Color[] rc = new Color[]{new Color(255,255,180,255), new Color(255,200,100,0)};
            RadialGradientPaint sun = new RadialGradientPaint(new Point2D.Double(sunX, sunY), 120f, rf, rc, MultipleGradientPaint.CycleMethod.NO_CYCLE);
            g.setPaint(sun);
            g.fill(new Ellipse2D.Double(sunX - 160, sunY - 160, 320, 320));

            // Rays
            g.setColor(new Color(255,255,180,20));
            g.setStroke(new BasicStroke(40f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            for (int i=0; i<10; i++) {
                double a = i * 0.35 + sunT * 3;
                double x2 = sunX + Math.cos(a) * 1400;
                double y2 = sunY + Math.sin(a) * 800;
                g.drawLine((int)sunX, (int)sunY, (int)x2, (int)y2);
            }
            ...
            // Pyramid
            drawPyramid(g, WIDTH * 0.7, FLOOR_Y - 40, 520, 260, shimmerT);

            // Foreground shrubs + sand
            drawForeground(g);
        ...
        private void drawPyramid(Graphics2D g, double cx, double baseY, double baseW, double height, double t) {
            float[] fr = new float[]{0f,1f};
            Color[] cs = new Color[]{new Color(180,160,120), new Color(120,100,70)};
            LinearGradientPaint stone = new LinearGradientPaint(0, (float)(baseY-height), 0, (float)baseY, fr, cs, MultipleGradientPaint.CycleMethod.NO_CYCLE);

            int steps = 14;
            for (int i=0;i<steps;i++) {
                double y = baseY - (i+1) * (height / steps);
                double w = baseW * (1 - (i+1.0)/ (steps+1.8));
                double x = cx - w/2.0;

                double wobble = Math.sin(t * 2.0 + i * 0.8) * 1.5;

                g.setPaint(stone);
                Polygon poly = new Polygon();
                poly.addPoint((int)x, (int)(y + wobble));
                poly.addPoint((int)(x+w), (int)(y + wobble));
                poly.addPoint((int)(x+w-8), (int)(y + (height/steps) + wobble));
                poly.addPoint((int)(x+8), (int)(y + (height/steps) + wobble));
                g.fill(poly);

                g.setColor(new Color(0,0,0,31));
                g.setStroke(new BasicStroke(2f));
                g.drawLine((int)x, (int)(y + wobble), (int)(x+w), (int)(y + wobble));
            }

            double topW = baseW * 0.18;
            double topH = height * 0.20;
            double topX = cx - topW/2;
            double topY = baseY - height - topH - 4;
            g.setColor(new Color(160, 130, 90));
            g.fillRect((int)topX, (int)topY, (int)topW, (int)topH);
            g.setColor(new Color(120, 95, 60));
            g.fillRect((int)(topX + topW*0.32), (int)(topY + topH*0.3), (int)(topW*0.36), (int)(topH*0.7));
        }

        private void drawForeground(Graphics2D g) {
            g.setColor(new Color(30, 60, 30, 140));
            for (int i=0;i<8;i++) {
                double x = i * WIDTH/8.0 + 40 * Math.sin(i);
                double w = 120 + (i%3)*40;
                g.fill(new Ellipse2D.Double(x, FLOOR_Y - 50 + Math.sin(i)*4, w, 40));
            }

            float[] fr = new float[]{0f, 1f};
            Color[] cs = new Color[]{new Color(180,150,90, 179), new Color(120, 90,50, 230)};
            LinearGradientPaint sand = new LinearGradientPaint(0, (float)FLOOR_Y, 0, HEIGHT, fr, cs, MultipleGradientPaint.CycleMethod.NO_CYCLE);
            g.setPaint(sand);
    ...
    static class Particle {
        ...
        static final AlphaComposite A1 = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1f);
