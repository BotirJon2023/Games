...
public class BoxingSimulationGame extends Canvas implements Runnable {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(() -> {
            JFrame frame = new JFrame("Boxing Simulation Game - London View (AWT/Swing)");
    ...
    class Background {
        ...
        private double eyeRotT = 0;
        private double flagT = 0;

        final double eyeX = WIDTH * 0.30;
        final double eyeY = FLOOR_Y - 120;
        final double bigBenX = WIDTH * 0.68;
        final double bigBenBaseY = FLOOR_Y - 50;

        Background() {
            Random r = new Random();
            for (int i=0;i<9;i++) clouds.add(new Cloud(rand(0, WIDTH), rand(60, 220), rand(90, 200), rand(0.06, 0.12)));
            for (int i=0;i<7;i++) birds.add(new Bird(rand(0, WIDTH), rand(140, 300), rand(60, 120), r.nextBoolean()));
            for (int i=0;i<3;i++) boats.add(new Boat(rand(-200, WIDTH), FLOOR_Y + 18 + i*14, rand(40, 70)));
            for (int i=0;i<120;i++) stars.add(new Star(rand(0, WIDTH), rand(30, 220), rand(0.3, 1.0)));
            for (int i=0;i<8;i++) {
                double y = FLOOR_Y - 24 + (i%2==0? 0 : 8);
                streaks.add(new Streak(rand(-300, WIDTH+300), y, rand(120, 220), rand(60, 140),
                        new Color(255, 220, 140, 120 + r.nextInt(70))));
            }
        }
        ...
        void update(double dt, double elapsed) {
            ...
            for (Boat b : boats) b.update(dt);
            for (Streak s : streaks) s.update(dt);

            for (Cloud c : clouds) if (c.x > WIDTH + 240) { c.x = -240; c.y = rand(60, 240); }
            for (Bird b : birds) {
                if (b.right && b.x > WIDTH + 100) { b.x = -100; b.y = rand(140, 300); }
                if (!b.right && b.x < -100) { b.x = WIDTH + 100; b.y = rand(140, 300); }
            }
            for (Boat b : boats) if (b.x > WIDTH + 260) { b.x = -260; }
            for (Streak s : streaks) if (s.x > WIDTH + 260) { s.x = -260; s.y = FLOOR_Y - 26 + rng.nextInt(18); }
        }
        ...
        void draw(Graphics2D g, double time, double shakeX, double shakeY) {
            ...
            drawSkylineSilhouette(g);
            drawLondonEye(g, eyeX, eyeY, 130, eyeRotT);
            drawElizabethTower(g, bigBenX, bigBenBaseY, 200, 300, time);
            drawWestminsterPalace(g, bigBenX - 320, bigBenBaseY - 110, 540, 120);

            drawThames(g);
            for (Boat b : boats) b.draw(g, waveT);
            for (Streak s : streaks) s.draw(g);

            // Waving flags near ring corners
            drawUnionJack(g, RING_LEFT - 30, FLOOR_Y - 160, 60, 36, flagT);
            drawUnionJack(g, RING_RIGHT + 30 - 60, FLOOR_Y - 160, 60, 36, flagT + 0.7);
        ...
        private void drawSky(Graphics2D g) {
            ...
            Color[] skyC = new Color[]{
                    new Color(16, 24, 50),   // upper twilight
                    new Color(50, 70, 120),  // mid
                    new Color(215, 140, 80)  // horizon glow
            };
        ...
        private void drawSkylineSilhouette(Graphics2D g) {
            // City of London skyline silhouettes
            g.setColor(new Color(25, 35, 55, 220));
            // The Shard
            drawShard(g, WIDTH*0.85, FLOOR_Y - 60, 120, 300);
            // 20 Fenchurch St (Walkie Talkie)
            drawWalkieTalkie(g, WIDTH*0.74, FLOOR_Y - 60, 120, 220);
            // 30 St Mary Axe (The Gherkin)
            drawGherkin(g, WIDTH*0.78, FLOOR_Y - 40, 80, 180);
            // St Paul's dome
            drawStPauls(g, WIDTH*0.60, FLOOR_Y - 44, 140, 120);

            // Distant block silhouettes (Canary Wharf-like)
            g.setColor(new Color(30, 40, 60, 200));
            for (int i=0;i<6;i++){
                int w = 40 + i*8;
                int h = 60 + i*10;
                int x = (int)(WIDTH*0.1 + i*70);
                g.fillRect(x, (int)(FLOOR_Y - 50 - h), w, h);
            }
        }

        private void drawShard(Graphics2D g, double bx, double baseY, double w, double h) {
            Path2D p = new Path2D.Double();
            p.moveTo(bx, baseY - h);
            p.lineTo(bx + w*0.25, baseY);
            p.lineTo(bx - w*0.18, baseY);
            p.closePath();
            float[] fr = new float[]{0f,1f};
            Color[] cs = new Color[]{new Color(80,100,140,200), new Color(40,50,80,220)};
            LinearGradientPaint lg = new LinearGradientPaint((float)bx, (float)(baseY - h), (float)(bx), (float)baseY, fr, cs, MultipleGradientPaint.CycleMethod.NO_CYCLE);
        ...
        private void drawWalkieTalkie(Graphics2D g, double cx, double baseY, double w, double h) {
            Path2D p = new Path2D.Double();
            p.moveTo(cx - w*0.4, baseY);
            p.lineTo(cx + w*0.35, baseY);
            p.lineTo(cx + w*0.48, baseY - h*0.9);
            p.quadTo(cx + w*0.50, baseY - h*1.02, cx - w*0.30, baseY - h);
            p.closePath();
            g.fill(p);
        }

        private void drawGherkin(Graphics2D g, double cx, double baseY, double w, double h) {
            Shape s = new Ellipse2D.Double(cx - w/2, baseY - h, w, h);
            g.fill(s);
            g.setColor(new Color(40, 50, 80, 140));
            g.setStroke(new BasicStroke(2f));
            for (int i=0;i<6;i++){
                double a = Math.toRadians(15 + i*25);
                g.draw(new Arc2D.Double(cx - w/2, baseY - h, w, h, 90 - Math.toDegrees(a), 180, Arc2D.OPEN));
                g.draw(new Arc2D.Double(cx - w/2, baseY - h, w, h, -90 + Math.toDegrees(a), 180, Arc2D.OPEN));
            }
        }

        private void drawStPauls(Graphics2D g, double cx, double baseY, double w, double h) {
            // Base
            g.fill(new Rectangle2D.Double(cx - w*0.45, baseY - h*0.25, w*0.9, h*0.25));
            // Dome
            g.setColor(new Color(40, 45, 70, 220));
            g.fill(new Arc2D.Double(cx - w*0.34, baseY - h*0.70, w*0.68, h*0.68, 0, 180, Arc2D.CHORD));
            g.fill(new Rectangle2D.Double(cx - 6, baseY - h*0.80, 12, h*0.10));
            g.fill(new Ellipse2D.Double(cx - 4, baseY - h*0.86, 8, 8));
        }

        private void drawLondonEye(Graphics2D g, double cx, double cy, double r, double rot) {
            // Supports
            g.setColor(new Color(70, 80, 100, 180));
            g.fill(new Rectangle2D.Double(cx - 6, cy, 12, r*0.6));
            Path2D leg = new Path2D.Double();
            leg.moveTo(cx, cy + r*0.6);
            leg.lineTo(cx - r*0.28, cy + r*0.2);
            leg.lineTo(cx - r*0.22, cy + r*0.14);
            leg.closePath();
            g.fill(leg);
            leg.reset();
            leg.moveTo(cx, cy + r*0.6);
            ...
            for (int i=0;i<spokes;i++){
                double a = rot + i * (2*Math.PI/spokes);
                double x = cx + Math.cos(a)*r;
                double y = cy + Math.sin(a)*r;
                g.setColor(new Color(200,210,230,180));
                g.draw(new Line2D.Double(cx, cy, x, y));
                // Capsule
                double ax = cx + Math.cos(a)*(r + 12);
                double ay = cy + Math.sin(a)*(r + 12);
                Shape cap = new RoundRectangle2D.Double(ax - 8, ay - 5, 16, 10, 6, 6);
                g.setColor(new Color(240,245,255,200));
                g.fill(cap);
                g.setColor(new Color(120, 140, 170, 120));
                g.draw(cap);
            }
        }

        private void drawElizabethTower(Graphics2D g, double cx, double baseY, double w, double h, double time) {
            // Lower palace connection block
            g.setColor(new Color(120, 80, 50, 180));
            g.fillRect((int)(cx - w*0.60), (int)(baseY - h*0.25), (int)(w*1.20), (int)(h*0.25));

            // Tower tiers
            g.setColor(new Color(160, 120, 80));
            g.fillRect((int)(cx - w*0.22), (int)(baseY - h*0.60), (int)(w*0.44), (int)(h*0.60));
            g.setColor(new Color(150, 110, 75));
            g.fillRect((int)(cx - w*0.18), (int)(baseY - h*0.74), (int)(w*0.36), (int)(h*0.14));

            // Clock face
            double clockR = w*0.11;
            double cy = baseY - h*0.58 + 4;
            ...
            g.setStroke(new BasicStroke(3f));
            linePolar(g, cx, cy, minuteAng, clockR * 0.82, new Color(240,210,120));
            g.setStroke(new BasicStroke(4f));
            linePolar(g, cx, cy, hourAng, clockR * 0.55, new Color(240,210,120));

            // Spire
            Path2D spire = new Path2D.Double();
            spire.moveTo(cx - w*0.12, baseY - h*0.74);
            spire.lineTo(cx + w*0.12, baseY - h*0.74);
            spire.lineTo(cx, baseY - h*0.96);
            spire.closePath();
            g.setColor(new Color(40, 100, 70));
            g.fill(spire);
            g.setColor(new Color(20, 60, 40));
            g.draw(spire);
        }

        private void drawWestminsterPalace(Graphics2D g, double x, double y, double w, double h) {
            g.setColor(new Color(110, 80, 55, 210));
            g.fill(new Rectangle2D.Double(x, y + h*0.4, w, h*0.6));
            // Roof crenellations
            g.setColor(new Color(100, 70, 50, 220));
            for (int i=0;i< (int)(w/16); i++){
                int bx = (int)(x + i*16);
                g.fillRect(bx, (int)(y + h*0.35 - (i%2==0? 6:0)), 12, 12);
            }
            // Windows glow
            for (int xx=0; xx<w; xx+=18) {
                g.setColor(new Color(255, 220, 140, 130 + (int)(60*Math.sin(twinkleT + xx*0.03))));
                g.fillRect((int)(x + 6 + xx), (int)(y + h*0.54 + (xx%36==0? -4: 0)), 8, 10);
            }
        }
        ...
        private void drawThames(Graphics2D g) {
            double y = FLOOR_Y;
            float[] fr = new float[]{0f, 1f};
            ...
            // Simple reflections of Eye and Big Ben (vertical streaks)
            g.setColor(new Color(160, 200, 220, 50));
            for (int i=0;i<6;i++){
                double ofs = Math.sin(waveT*1.1 + i*0.7)*14;
                g.fill(new Rectangle2D.Double(eyeX + ofs - 2 - i, y + 6 + i*10, 4 + i*1.2, 80 + i*10));
            }
            for (int i=0;i<6;i++){
                double ofs = Math.sin(waveT*1.3 + i*0.5)*16;
                g.fill(new Rectangle2D.Double(bigBenX + ofs - 2 - i, y + 6 + i*10, 4 + i*1.2, 90 + i*12));
            }
        }

        private void drawUnionJack(Graphics2D g, double x, double y, double w, double h, double t) {
            // Pole
            g.setColor(new Color(200,200,200));
            g.fill(new Rectangle2D.Double(x - 2, y - 4, 3, h + 28));
            // Wave offset
            double wave = Math.sin(t) * 6;

            // Base blue field
            Path2D field = new Path2D.Double();
            field.moveTo(x, y);
            field.curveTo(x + w*0.35, y + wave*0.3, x + w*0.70, y - wave*0.4, x + w, y + wave*0.2);
            field.lineTo(x + w, y + h + wave*0.2);
            field.curveTo(x + w*0.70, y + h - wave*0.4, x + w*0.35, y + h + wave*0.3, x, y + h);
            field.closePath();
            g.setColor(new Color(20,40,130));
            g.fill(field);
            ...
            // Central white cross
            g.setColor(Color.WHITE);
            g.fill(new Rectangle2D.Double(x + w*0.48, y, w*0.04, h));
            g.fill(new Rectangle2D.Double(x, y + h*0.46, w, h*0.08));
            // Central red cross
            g.setColor(new Color(200,30,30));
            g.fill(new Rectangle2D.Double(x + w*0.49, y, w*0.02, h));
            g.fill(new Rectangle2D.Double(x, y + h*0.47, w, h*0.06));
        }
        ...
        class Boat {
            double x,y,speed;
            Boat(double x,double y,double speed){this.x=x;this.y=y;this.speed=speed;}
            void update(double dt){ x += speed * dt; }
            void draw(Graphics2D g, double wav){
                g.setColor(new Color(200, 200, 210, 200));
                g.fill(new RoundRectangle2D.Double(x, y - 10, 36, 10, 6, 6));
                g.setColor(new Color(140, 150, 170, 220));
                g.fill(new Rectangle2D.Double(x + 8, y - 18, 18, 8));
                // Wake
                g.setColor(new Color(200,220,240,80));
                Path2D wake = new Path2D.Double();
                wake.moveTo(x + 36, y);
                wake.lineTo(x + 56, y + 4 + Math.sin(wav + x*0.02)*2);
                wake.lineTo(x + 36, y + 6);
                wake.closePath();
                g.fill(wake);
            }
        }
